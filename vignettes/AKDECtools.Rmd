...
| MagDurFreq_turbidity | turbidity_sites | turbidity_sites.csv | Method, Group, SiteID, AUID, Role |
...
| MagDurFreq_combine | pathogens_output | NA | *Output from MagDurFreq_pathogens* |
| categorize_AU_uses | MagDurFreq_Results | *NA* | *Output from MagDurFreq_combine* |
| categorize_AU_uses | assessments | assessments.csv | Download from ATTAINS |
...
MagDurFreq_turbidity(df_WQS_Crosswalk, input_samples_filtered, df_Data_Sufficiency, df_Turbidity_Sites)
```

### Pathogens

The *MagDurFreq_pathogens* function applies pathogen water quality standards. Inputs include the pathogen water quality data and data sufficiency results from *data_processing.R*, the data-sufficient water quality data generated using *filterCat3samples*, and the WQS crosswalk table. See function documentation by running `?AKDECtools::MagDurFreq_pathogens`.

See below for an example of the *MagDurFreq_pathogens* function.
...
# run MagDurFreq_turbidity
MagDurFreq_pathogens(input_samples_filtered, df_WQS_Crosswalk, df_Data_Sufficiency)
```

### Combine

The *MagDurFreq_combine* function combines MagDurFreq results from all five MagDurFreq functions. See function documentation by running `?AKDECtools::MagDurFreq_combine`.
...
pathogens_output <-  MagDurFreq_pathogens(input_samples_filtered, df_WQS_Crosswalk
                                          , df_Data_Sufficiency)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Example 1
uses_categorized <- categorize_AU_uses(df_MagDurFreq_Results, df_Assessments, simplify_standards = F)
```
